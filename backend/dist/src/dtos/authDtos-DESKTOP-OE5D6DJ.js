...
exports.AuthDtoValidator = void 0;
...
class AuthDtoValidator {
    validateSignup(body) {
        ...
        if (name.length < config_js_1.AUTH_LIMITS.MIN_NAME_LENGTH) {
            throw new Error(messages_js_1.AUTH_MESSAGES.NAME_TOO_SHORT);
        }
    ...
    assertEmail(email) {
        if (!/^\S+@\S+\.\S+$/.test(email)) {
            throw new Error(messages_js_1.AUTH_MESSAGES.INVALID_EMAIL);
        }
    }
    assertPassword(password) {
        if (password.length < config_js_1.AUTH_LIMITS.MIN_PASSWORD_LENGTH) {
            throw new Error(messages_js_1.AUTH_MESSAGES.PASSWORD_TOO_SHORT);
        }
    }
    assertOtp(otp) {
        if (!/^\d{6}$/.test(otp)) {
...
exports.AuthDtoValidator = AuthDtoValidator;
exports.default = new AuthDtoValidator();
